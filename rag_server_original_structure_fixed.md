...
다음 조건에서 query rewriting을 수행한다.

- 질문이 짧고 문맥 의존적인 경우 (예: “그럼 배송비는?”)
- 이전 질문을 참조하는 지시어를 포함하는 경우 (예: “그건”, “그럼”, “이건”)
- 멀티턴 대화에서 의미가 불완전한 경우

## External API Policy

- LLM timeout: 3~5초
- Embedding timeout: 3초
- 외부 API 호출 실패 시 최대 2회 retry를 수행한다.

## Prompt Loading Policy

- 프롬프트는 `resources/prompts` 경로에서 로딩한다.
- 프롬프트는 애플리케이션 시작 시 메모리에 캐싱한다.
- 프롬프트 문자열은 코드에 하드코딩하지 않는다.
...
#### Request Rules
- `session_id`는 optional이다.
- `session_id`가 없으면 서버가 새로운 세션을 생성한다.
- `question`은 필수값이다.
...
- 인덱싱 작업은 비동기(Spring Async)로 수행된다.
...
#### Rules
- 동일 문서를 다시 업로드하면 `document_version`을 증가시킨다.
